...
class CanDBCNode(Node):
    def __init__(self):
        super().__init__("can_dbc_to_topics")
        ...
        for message in self.db.messages:
            for signal in message.signals:
                ...
                if signal.name in SIGNAL_MAP:
                    ...
                    self.get_logger().info(f"Publishing {signal.name} on {topic_name}")
    ...
    def configure_can_interface(self):
        ...
        """
        try:
            self.get_logger().info("Configuring can0 interface with 500000 bitrate...")
            ...
            self.get_logger().info("can0 interface configured successfully.")
        except subprocess.CalledProcessError as e:
            self.get_logger().error(f"Error in configuring can0: {e}")
            exit(1)
    ...
    def rx_loop(self):
        ...
        while self.running and rclpy.ok():
            ...
            try:
                ...
                self.get_logger().warn(f"Decode fail ID=0x{msg.arbitration_id:X}: {e}")
                continue
...
def main():
    ...
    try:
        rclpy.spin(node)  # ROS1에서 메시지 처리 및 대기
